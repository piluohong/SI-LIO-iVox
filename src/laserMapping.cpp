...
pcl::VoxelGrid<PointType> downSizeFilterMap;
      ...
      using IVoxType = faster_lio::IVox<3, faster_lio::IVoxNodeType::PHC, PointType>; // 伪希尔伯特空间填充曲线进行查询近邻点
#else
...
bool first_flg = true;
int ivox_nearby_type_ = 18;
float ivox_map_size_ = 0.;
KD_TREE<PointType> ikdtree;
IVoxType::Options ivox_options_;
std::shared_ptr<IVoxType> ivox_ = nullptr;

void set_ivox_option()
{
    if (ivox_nearby_type_ == 0) {
            ivox_options_.nearby_type_ = IVoxType::NearbyType::CENTER;
        } else if (ivox_nearby_type_ == 6) {
...
void map_incremental_ivox()
{
      ...
      for (int i = 0; i < feats_down_size; i++)
      {
        ...
        if (!Nearest_Points[i].empty() && flg_EKF_inited) {
            const PointVector &points_near = Nearest_Points[i];
            PointType mid_point;
            mid_point.x = floor(point_world.x / ivox_map_size_) * ivox_map_size_ + 0.5 * ivox_map_size_;
            mid_point.y = floor(point_world.y / ivox_map_size_) * ivox_map_size_ + 0.5 * ivox_map_size_;
            mid_point.z = floor(point_world.z / ivox_map_size_) * ivox_map_size_ + 0.5 * ivox_map_size_;

            if (fabs(points_near[0].x - mid_point.x) > 0.5 * ivox_map_size_ 
                && fabs(points_near[0].y - mid_point.y) > 0.5 * ivox_map_size_
                && fabs(points_near[0].z - mid_point.z) > 0.5 * ivox_map_size_)
            {
            ...
            bool need_add = true;
            float dist = calc_dist(point_world, mid_point);
            if (points_near.size() >= NUM_MATCH_POINTS) {
                for (int readd_i = 0; readd_i < NUM_MATCH_POINTS; readd_i++) {
                    if (calc_dist(points_near[readd_i], mid_point) < dist + 1e-6) {
                        need_add = false;
                        break;
                    }
                }
...
int main(int argc, char **argv)
{
    ...
    nh.param<int>("ivox/ivox_nearby_type", ivox_nearby_type_, 18);
    nh.param<bool>("ivox/use_ivox", use_ivox_, false);
    nh.param<float>("ivox/ivox_map_size", ivox_map_size_, 0.5);
    ...
    while (ros::ok())
    {
        ...
        if (sync_packages(Measures)) //把一次的IMU和LIDAR数据打包到Measures
        {
            ...
            if(use_ivox_)
            {
                kf.update_iterated_dyn_share_modified(LASER_POINT_COV, feats_down_body, ivox_, Nearest_Points, NUM_MAX_ITERATIONS, extrinsic_est_en, iter_num);
            }else
    ...
    float error = std::sqrt(std::pow(path.poses.back().pose.position.x - path.poses.front().pose.position.x,2) + 
    std::pow(path.poses.back().pose.position.y - path.poses.front().pose.position.y,2) + 
    std::pow(path.poses.back().pose.position.z - path.poses.front().pose.position.z,2));
    std::cout << "End-to-end deviation : " << error << " (m)" << std::endl;
